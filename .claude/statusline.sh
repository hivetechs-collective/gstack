...
# (2026-09-02).
# Start EMPTY: never trust an inherited PROJECT_ROOT. .claude/lib/config.sh
# exports it for every hook that sources it, so any process a hook spawns (the
# 2.51.1 post-push confirm) carries the main checkout's path; outside a git repo
# the fallback below then kept that path instead of $PWD, and every sandboxed
# render read the checkout's helpers and caches (1.8.1, /plan-w-team 2.51.4).
PROJECT_ROOT=""
...
# A reset that is NOT today needs its weekday: a weekly bucket that resets
# "2:00am" five days out reads as tonight. Same local day ⇒ "6:00pm";
# otherwise "Wed 9/23 2:00am" (1.6.0 — the Fable bucket's reset). The calendar
# date rides along (1.8.0): a bare "Sun" does not say WHICH Sunday, and a sample
# read late in its window can be a day or a week out. US month/day to match the
# 12-hour clock; leading zeros stripped in shell (no non-portable %-m / %-d).
fmt_time_dhm() {
  local epoch="$1" d_then d_now dow mon day
  if date -r 0 +%s >/dev/null 2>&1; then
    d_then=$(date -r "$epoch" +%Y%m%d); dow=$(date -r "$epoch" +%a)
  else
    d_then=$(date -d "@$epoch" +%Y%m%d); dow=$(date -d "@$epoch" +%a)
  fi
  d_now=$(date +%Y%m%d)
  if [ "$d_then" = "$d_now" ]; then fmt_time_hm "$epoch"; return 0; fi
  mon="${d_then:4:2}"; day="${d_then:6:2}"
  printf '%s %s/%s %s' "$dow" "${mon#0}" "${day#0}" "$(fmt_time_hm "$epoch")"
}
...
if [ "$HAS_JQ" -eq 1 ]; then
  ...
  seven_reset_ep="${h_seven_reset:-$sd_seven_reset}"
  seven_reset_shown=0
  ...
  if [ -n "$seven_d" ]; then
    ...
    # The weekly window resets days out, so it gets the Fable bucket's form
    # (2.54.0): "7d 71% (resets Wed 9/30 3:59am)", a bare clock time when the
    # reset is later today. Future resets only — a passed one is a stale sample.
    case "$seven_reset_ep" in ''|*[!0-9]*) ;; *)
      if [ "$seven_reset_ep" -gt "$now_s" ]; then
        LIM_OUT="$LIM_OUT $(C '38;5;245')(resets $(fmt_time_dhm "$seven_reset_ep"))$(rst)"
        seven_reset_shown=1
      fi ;;
    esac
  ...
  if [ -n "$plan_json" ]; then
    sc_lines=$(printf '%s' "$plan_json" | jq -r '. as $d | .limits[]? | select(.kind == "weekly_scoped") | (((.scope.model.display_name) // "?") as $n | [$n, ((.percent // "") | tostring), (if .is_active == true then "1" else "0" end), (.severity // ""), (($d._meta.rates.scoped[$n].eta_s // "") | tostring), ((.resets_at // "") | if . == "" then "" else (tostring | sub("\\.[0-9]+"; "") | sub("\\+00:00$"; "Z") | (try fromdateiso8601 catch "") | tostring) end)] | join("|"))' 2>/dev/null)
    while IFS='|' read -r sc_name sc_pct sc_active sc_sev sc_eta sc_reset; do
      [ -n "$sc_name" ] && [ -n "$sc_pct" ] || continue
      lim_seg "$sc_name" "$sc_pct" "$sc_eta" "" "$sc_sev" "$sc_active"
      # When the bucket resets (1.6.0): a lane that NEEDS this model waits on this
      # instant, not on the 5h one. Future resets only — a passed one is a stale sample.
      # (2.54.0) The scoped bucket shares the weekly anchor on every account seen so
      # far, so its reset is shown only when it DIFFERS from the one already on the
      # 7d gauge — the same date twice on one line is noise.
      case "$sc_reset" in ''|*[!0-9]*) ;; *)
        if [ "$sc_reset" -gt "$now_s" ] && ! { [ "$seven_reset_shown" = "1" ] && same_window "$seven_reset_ep" "$sc_reset"; }; then
          LIM_OUT="$LIM_OUT $(C '38;5;245')(resets $(fmt_time_dhm "$sc_reset"))$(rst)"
        fi ;;
      esac
      if [ -n "$line2" ]; then line2="$line2 $(C '38;5;245')·$(rst) $LIM_OUT"; else line2="📊 $(C '38;5;117')Plan:$(rst) $LIM_OUT"; fi
    done <<< "$sc_lines"
...
if [ "$PWT_LEAN" -eq 0 ] && [ -x "$ACCOUNT_ADVICE_HELPER" ] && [ "$HAS_JQ" -eq 1 ] && [ -n "$line2" ]; then
  # The session's model rides along: a Fable pane asks for a Fable-aware pick
  # (rank on max(5h,7d,Fable), Fable-rejected accounts excluded, own cache file).
  advice_json=$("$ACCOUNT_ADVICE_HELPER" "${acct_email:-}" "--model=${model_id:-$model_name}" 2>/dev/null)
  if [ -n "$advice_json" ] && [ "$advice_json" != "{}" ]; then
    adv_switch=$(echo "$advice_json" | jq -r '.switch // false' 2>/dev/null)
    # Reset-aware (1.7.0 / skill 2.45.0): the next Fable opening among Fable-shut
    # accounts. With NO pick (a Fable pane, every Fable bucket shut) it is the whole
    # message — wait for it, or work on Opus until it opens. Beside a pick it is a
    # grey tail, shown only when the opening is within 12 h (soon enough to plan on).
    adv_open_tail=""
    adv_open_row=$(echo "$advice_json" | jq -r '[(.fable_next_open.at_epoch // ""), (.fable_next_open.label // ""), (.fable_wait // "")] | map(tostring) | join("|")' 2>/dev/null)
    IFS='|' read -r adv_open_at adv_open_label adv_open_wait <<< "${adv_open_row:-||}"
    case "$adv_open_at" in ''|*[!0-9]*) adv_open_at="" ;; esac
    if [ -n "$adv_open_at" ] && [ "$adv_open_at" -gt "$(date +%s)" ] 2>/dev/null; then
      adv_open_txt="Fable opens $(fmt_time_dhm "$adv_open_at")${adv_open_label:+ on $adv_open_label}"
      case "$adv_open_wait" in
        hold)      line2="$line2 $(C '38;5;245')·$(rst) $(C "$(tier_color warm)")⏳ ${adv_open_txt} · hold for it$(rst)" ;;
        downgrade) line2="$line2 $(C '38;5;245')·$(rst) $(C "$(tier_color warm)")⏳ ${adv_open_txt} · use Opus until then$(rst)" ;;
        *) [ $(( adv_open_at - $(date +%s) )) -le 43200 ] && adv_open_tail=" $(C '38;5;245')· ${adv_open_txt}$(rst)" ;;
      esac
    fi
    if [ "$adv_switch" = "true" ]; then
      ...
      if [ -n "$adv_who" ]; then
        # The pick's readings, LABELLED, all three windows: "(5h 0% · 7d 7% · Fable 95%)".
        # It used to print a bare "(0/7%)" — two unlabelled numbers the operator had
        # to decode, and no Fable bucket at all, so an account at 0/7% could be
        # recommended while sitting at 95 % of its Fable week (2026-09-18). The
        # scoped names come from the advisory (`best_scoped`, {display_name: pct})
        # so a new model bucket shows up without a statusline edit.
        adv_pct=""
        adv_parts=""
        if [ -n "$adv_b5" ]; then
          adv_b5f=$(printf '%.0f' "$adv_b5" 2>/dev/null || echo "$adv_b5")
          adv_parts="5h ${adv_b5f}%"
        fi
        if [ -n "$adv_b7" ]; then
          adv_b7f=$(printf '%.0f' "$adv_b7" 2>/dev/null || echo "$adv_b7")
          adv_parts="${adv_parts:+$adv_parts · }7d ${adv_b7f}%"
        fi
        # …each with its bucket status (1.5.0): `⛔` when the bucket is rejected,
        # `~` when the number is a plan-usage sample rather than a live header
        # reading, so "Fable 95%" can never again be a previous week's sample
        # wearing a live face (2026-09-18).
        adv_sc=$(echo "$advice_json" | jq -r '. as $a | (.best_scoped // {}) | to_entries[] | select(.value != null) | "\(.key)|\(.value)|\(($a.best_scoped_status // {})[.key] // "")|\($a.best_scoped_source // "")"' 2>/dev/null)
        if [ -n "$adv_sc" ]; then
          while IFS='|' read -r adv_sc_name adv_sc_pct adv_sc_st adv_sc_src; do
            [ -n "$adv_sc_name" ] || continue
            adv_sc_f=$(printf '%.0f' "$adv_sc_pct" 2>/dev/null || echo "$adv_sc_pct")
            adv_sc_tag=""
            case "$adv_sc_st" in rejected) adv_sc_tag=" ⛔" ;; esac
            case "$adv_sc_src" in plan-usage-sample) adv_sc_tag="${adv_sc_tag} ~" ;; esac
            adv_parts="${adv_parts:+$adv_parts · }${adv_sc_name} ${adv_sc_f}%${adv_sc_tag}"
          done <<< "$adv_sc"
        fi
        [ -n "$adv_parts" ] && adv_pct=" (${adv_parts})"
        ...
        case "$adv_tier" in
          hot)  line2="$line2 $(C '38;5;245')·$(rst) $(C "$(tier_color hot)")⚠ switch → ${adv_who}${adv_pct}$(rst)${adv_open_tail}" ;;
          warm) line2="$line2 $(C '38;5;245')·$(rst) $(C "$(tier_color warm)")👉 next: ${adv_who}${adv_pct}$(rst)${adv_open_tail}" ;;
          # cool keeps the advisory's muted green: an idle nudge stays quieter than
          # the plan number it sits beside.
          *)    line2="$line2 $(C '38;5;245')·$(rst) $(C '38;5;108')👉 next: ${adv_who}${adv_pct}$(rst)${adv_open_tail}" ;;
        esac
      fi
    fi
  fi
...
# ---- Prompt-cache segment (1.4.0) ----
# The `/cost` "Prompt cache (main)" line, rendered live from the stdin
# `prompt_cache` object (CLI ≥ 2.1.251; `last_miss_cause` ≥ 2.1.260). Main
# conversation only; absent until the first API response, so the segment
# simply does not render on a fresh session. Pure stdin: one jq spawn, no
# helper, no transcript parsing — cheap enough to keep in lean mode.
#
#   💾 Cache: 91% hit · ▸warm 1h (cold in 42m) · 2 misses (last 6m ago: tools changed) · 1 rebuild
#   💾 Cache: 91% hit · ⚠ COLD · next msg re-caches 310k tok
#   💾 Cache: none observed
#
# Why it earns a line: on Max the 5h window is spent in input tokens, and a
# cold cache re-bills the whole prompt floor (74–82K in this repo, 2.41.0) on
# the next message. "cold in 42m" is the one number that tells you whether
# stepping away costs a full re-cache; the miss cause tells you WHY the last
# rebuild happened (tools changed / system prompt changed / TTL) so a hook or
# MCP server that churns the prompt can be found instead of guessed at.
# No $-figures anywhere (memory: feedback_statusline_no_dollar_cost).
# Kill switch: STATUSLINE_DISABLE_CACHE_SEGMENT=1.
fmt_rel_s() {
  # 40 → "40s", 370 → "6m", 4320 → "1h 12m", 90000 → "1d 1h"
  local s="$1" d h m
  [[ "$s" =~ ^[0-9]+$ ]] || { printf '?'; return; }
...
fmt_tok_k() {
  # 310200 → "310k", 1234567 → "1.2M", 900 → "900"
  local n="$1"
  [[ "$n" =~ ^[0-9]+$ ]] || { printf '?'; return; }
...
cache_miss_label() {
  # Machine cause → words. Unknown causes fall through with underscores spaced.
  case "$1" in
    tools_changed)         printf 'tools changed' ;;
    system_prompt_changed) printf 'system prompt changed' ;;
    ttl_expired_5m)        printf '5m TTL expired' ;;
    ttl_expired_1h)        printf '1h TTL expired' ;;
    ttl_expired*)          printf 'TTL expired' ;;
    likely_server_side)    printf 'server-side' ;;
    "")                    printf '' ;;
    *)                     printf '%s' "$1" | tr '_' ' ' ;;
  esac
}
cache_seg=""
if [ "$HAS_JQ" -eq 1 ] && [ "${STATUSLINE_DISABLE_CACHE_SEGMENT:-0}" != "1" ]; then
  # One jq call → one pipe-delimited row. `|` is safe: every field is a number,
  # a bool, a fixed enum, or a snake_case cause token.
  pc_row=$(echo "$input" | jq -r '
    (.prompt_cache // empty) | select(type == "object")
    | [ (.requests // 0), (.misses // 0), (.expected_rebuilds // 0),
        (if .hit_ratio == null then "" else .hit_ratio end),
        (if .warm == true then 1 else 0 end),
        (if .caching_observed == false then 0 else 1 end),
        (.ttl // ""), (.expires_at // ""), (.last_miss_at // ""),
        ((.last_miss_cause.causes // []) | first // ""),
        (.recache_tokens_if_cold // "") ]
    | map(tostring) | join("|")' 2>/dev/null)
  if [ -n "$pc_row" ]; then
    IFS='|' read -r pc_req pc_miss pc_rebuild pc_hit pc_warm pc_obs pc_ttl pc_exp pc_lastmiss pc_cause pc_recache <<< "$pc_row"
    if [[ "$pc_req" =~ ^[0-9]+$ ]] && [ "$pc_req" -gt 0 ]; then
      pc_now=$(date +%s)
      pc_dim="$(C '38;5;245')"
      if [ "$pc_obs" = "0" ]; then
        cache_seg="💾 $(C '38;5;117')Cache:$(rst) ${pc_dim}none observed$(rst)"
      else
        # Hit ratio: ≥90 mint, 75–89 peach, <75 coral — same ladder as context.
        pc_hit_txt=""
        if [[ "$pc_hit" =~ ^[0-9.]+$ ]]; then
          pc_hit_pct=$(awk -v r="$pc_hit" 'BEGIN{printf "%d", r*100 + 0.5}')
          if [ "$pc_hit_pct" -ge 90 ]; then pc_hit_col="$(C '38;5;158')"
          elif [ "$pc_hit_pct" -ge 75 ]; then pc_hit_col="$(C '38;5;215')"
          else pc_hit_col="$(C '38;5;203')"; fi
          pc_hit_txt="${pc_hit_col}${pc_hit_pct}% hit$(rst)"
        else
          pc_hit_txt="${pc_dim}hit n/a$(rst)"
        fi
        # Warm/cold: the number that says whether stepping away costs a re-cache.
        if [ "$pc_warm" = "1" ]; then
          pc_state="$(C '38;5;158')▸warm${pc_ttl:+ $pc_ttl}$(rst)"
          if [[ "$pc_exp" =~ ^[0-9]+$ ]]; then
            pc_left=$(( pc_exp - pc_now ))
            if [ "$pc_left" -gt 0 ]; then
              pc_left_col="$(C '38;5;158')"
              [ "$pc_left" -lt 300 ] && pc_left_col="$(C '38;5;215')"
              pc_state="$pc_state ${pc_left_col}(cold in $(fmt_rel_s "$pc_left"))$(rst)"
            else
              pc_state="$(C '38;5;215')▸expiring$(rst)"
            fi
          fi
        else
          pc_state="$(C '38;5;215')⚠ COLD$(rst)"
        fi
        cache_seg="💾 $(C '38;5;117')Cache:$(rst) ${pc_hit_txt} ${pc_dim}·$(rst) ${pc_state}"
        # Misses, with the last one's age and cause.
        if [[ "$pc_miss" =~ ^[0-9]+$ ]] && [ "$pc_miss" -gt 0 ]; then
          pc_miss_txt="${pc_miss} miss"; [ "$pc_miss" -ne 1 ] && pc_miss_txt="${pc_miss_txt}es"
          pc_detail=""
          if [[ "$pc_lastmiss" =~ ^[0-9]+$ ]] && [ "$pc_lastmiss" -le "$pc_now" ]; then
            pc_detail="last $(fmt_rel_s $(( pc_now - pc_lastmiss ))) ago"
          fi
          pc_cause_txt=$(cache_miss_label "$pc_cause")
          if [ -n "$pc_cause_txt" ]; then
            pc_detail="${pc_detail:+$pc_detail: }${pc_cause_txt}"
          fi
          [ -n "$pc_detail" ] && pc_miss_txt="${pc_miss_txt} ${pc_dim}(${pc_detail})$(rst)"
          cache_seg="$cache_seg ${pc_dim}·$(rst) ${pc_miss_txt}"
        fi
        # Expected rebuilds (compaction / tool-result clearing) are not misses.
        if [[ "$pc_rebuild" =~ ^[0-9]+$ ]] && [ "$pc_rebuild" -gt 0 ]; then
          pc_rb_txt="${pc_rebuild} rebuild"; [ "$pc_rebuild" -ne 1 ] && pc_rb_txt="${pc_rb_txt}s"
          cache_seg="$cache_seg ${pc_dim}·$(rst) ${pc_dim}${pc_rb_txt}$(rst)"
        fi
        # Cold: what the next message re-caches. Null right after compaction.
        if [ "$pc_warm" != "1" ] && [[ "$pc_recache" =~ ^[0-9]+$ ]] && [ "$pc_recache" -gt 0 ]; then
          cache_seg="$cache_seg ${pc_dim}·$(rst) $(C '38;5;215')next msg re-caches $(fmt_tok_k "$pc_recache") tok$(rst)"
        fi
      fi
    fi
  fi
...
if [ -n "$cache_seg" ]; then
  if [ -n "$line3" ]; then line3="$line3  $cache_seg"; else line3="$cache_seg"; fi
fi
