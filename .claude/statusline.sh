  ...
  if [ -n "$advice_json" ] && [ "$advice_json" != "{}" ]; then
    ...
    if [ "$adv_switch" = "true" ]; then
      ...
      if [ -n "$adv_who" ]; then
        ...
        adv_sc=$(echo "$advice_json" | jq -r '(.best_scoped // {}) | to_entries[] | select(.value != null) | "\(.key)|\(.value)"' 2>/dev/null)
        if [ -n "$adv_sc" ]; then
          while IFS='|' read -r adv_sc_name adv_sc_pct; do
            [ -n "$adv_sc_name" ] || continue
            adv_sc_f=$(printf '%.0f' "$adv_sc_pct" 2>/dev/null || echo "$adv_sc_pct")
            adv_parts="${adv_parts:+$adv_parts · }${adv_sc_name} ${adv_sc_f}%"
          done <<< "$adv_sc"
        fi
