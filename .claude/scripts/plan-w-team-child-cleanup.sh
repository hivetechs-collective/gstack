...
jq -nc \
    ...
    '{skipped:false, registered:$registered, stopped:$stopped, noop:$noop, attempts:$attempts}' \
    2>/dev/null || printf '{"skipped":false,"registered":%d,"stopped":%d,"noop":%d,"attempts":[]}\n' \
        "$TOTAL_REGISTERED" "$TOTAL_STOPPED" "$TOTAL_NOOP"
