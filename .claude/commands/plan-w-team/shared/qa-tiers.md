...
| Step 2 Task breakdown   | Paired test tasks (`N.a`) own the T1/T2 rows for their area — applies to UI scope AND non-UI code-adding scopes (BACKEND/INFRASTRUCTURE/SCRIPTS/LIBRARY/API) per STE extension. Refactor/docs/config remain single-task. Hot-path files get an optional `N.p` benchmark slot (perf-testing-specialist); one-way-door tasks get `mutation_required: true` (TO2 default-on). |
...
## Non-UI Paired Tasks (STE Extension)

The N.a/N.b paired-task discipline historically applied to UI features only — UI shipped behind paired Playwright contracts while non-UI features shipped as single tasks with "tests added later." That gap was closed by the 2026-05 STE extension. Now:

| Scope                                                            | N.a/N.b paired? | N.a agent default                                                          | N.b agent default                                                                                                    |
| ---------------------------------------------------------------- | --------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| UI / FRONTEND                                                    | yes             | `stagehand-expert` or `unit-testing-specialist`                            | framework specialist (`react-typescript-specialist` / `vue-specialist` / `svelte-specialist` / `angular-specialist`) |
| BACKEND / INFRASTRUCTURE / SCRIPTS / LIBRARY / API (mode == add) | yes             | `unit-testing-specialist` (or language-specific test specialist if avail.) | language/framework specialist (`nodejs-specialist`, `rust-backend-specialist`, `fastapi-specialist`, etc.)           |
| Refactor-only (any scope)                                        | no              | —                                                                          | original specialist; existing tests constrain                                                                        |
| Docs-only                                                        | no              | —                                                                          | `documentation-expert`                                                                                               |
| Config-only (env, build flags)                                   | no              | —                                                                          | original specialist; pure config has no behavioral surface                                                           |
| DATABASE schema migrations                                       | no              | —                                                                          | covered by Step 5 one-way-door reviewer scrutiny instead of N.a/N.b                                                  |

Optional overlays Step 2 may emit when applicable:

- `N.p` — `perf-testing-specialist` benchmark for **hot-path** files (>1000 LOC OR >50 commits in last 30 days). Optional. Mandatory only when the parent task is also tagged `door_type: one-way`.
- `N.c` — retroactive coverage task created in Step 5 from the `test-gap-analyzer` report (assigned to `unit-testing-specialist`). Runs after Step 6 ship and before Step 8 retro.
