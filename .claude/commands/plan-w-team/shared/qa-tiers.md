...
| ---------------------------------------------------------------- | --------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| UI / FRONTEND                                                    | yes             | `stagehand-expert` or `unit-testing-specialist` | `react-typescript-specialist` for React/TSX, else `builder` — the `frontend-web` skill auto-triggers               |
| BACKEND / INFRASTRUCTURE / SCRIPTS / LIBRARY / API (mode == add) | yes             | `unit-testing-specialist`                       | `rust-backend-specialist` for Rust, else `builder` / `builder-opus` — the `backend-frameworks` skill auto-triggers |
| Refactor-only (any scope)                                        | no              | —                                               | original lane; existing tests constrain                                                                            |
| Docs-only                                                        | no              | —                                               | `documentation-expert`                                                                                             |
| Config-only (env, build flags)                                   | no              | —                                               | original lane; pure config has no behavioral surface                                                               |
...
- `N.p` — `performance-testing-specialist` benchmark for **hot-path** files (>1000 LOC OR >50 commits in last 30 days). Optional. Mandatory only when the parent task is also tagged `door_type: one-way`.
