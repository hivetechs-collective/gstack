...
- **PWT-DS1** (process-level, mid-2026-05; liveness tier 2026-08-14): even if the assistant ignores the systemMessage marker and calls `pwt-goal.sh` (`--worker-only` or `--launch`) anyway, the script checks `.claude/state/plan-w-team-hook-spawn-<sid>.flag` (written by the hook on successful spawn) and refuses → exit 3 (`PWT_DS1_DUPLICATE`) via two tiers: **Tier A** — a flag fresh within `PWT_DOUBLE_SPAWN_WINDOW_MIN` (default 3 min, mtime-based); **Tier B (PWT-DS1-LIVE)** — beyond that window, the recorded worker is still live in `claude agents --json` (the plan-mode-delay case, where a long plan-mode pause ages the flag out of Tier A but the worker keeps running — field incident 2026-08-14). Tier B fail-safe: only _adds_ refusals on positive liveness; indeterminate → proceed + warn. Kill switch `PWT_DOUBLE_SPAWN_LIVENESS_DISABLE=1`; `PLAN_W_TEAM_FORCE_SPAWN=1` bypasses both. The flag-file is registered in `shared/state-artifacts.md`.
...
5. `.claude/scripts/plan-w-team-test-green.sh --check --slug <SLUG>` — **ADVISORY** skill-suite verdict (exit 0 green / 1 red / 2 no-artifact yet). Never runs the suite, so a tick stays cheap. A **red** verdict during execute is a **Fix-Now signal** (feed it to item 5), not a halt — the worker may legitimately be mid-fix, and the ENFORCING gate is Step 6 ship. Exit 2 (no artifact) and a missing script are both normal: pre-ship runs and consumer repos without the harness have no verdict to read — no-op, never a block.
6. **Fix-Now Audit** — scan the tick's worker log + persisted review findings for any defect or flaky test logged as merely **"noted"** (or a flaky "fixed" by loosen/retry/`.skip`/timeout-widen). See Fix-Now Audit below.
