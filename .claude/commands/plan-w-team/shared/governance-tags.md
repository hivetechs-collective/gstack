...
| Stale-state janitor delete set                | `.claude/scripts/plan-w-team-cleanup-stale-goal-states.sh`                                                               | The PASS-2 broom deletes per-run `.claude/state` families on every session-start auto-run. Its `PER_SLUG_REAP_PREFIXES`, `GLOBAL_DENYLIST`, and classification (`__slug_of_file` longest-prefix-wins + charset guard) decide the delete blast radius: a wrong reap prefix, a dropped denylist entry, or a weakened classification deletes a LIVE run's state or a durable cross-run global (e.g. `plan-w-team-retro-capture-history.jsonl`). Parity is pinned by `plan-w-team-cleanup-registry-parity.test.sh`; any diff to this script forces a Step-5 security review.                                                                                                                                                         |
