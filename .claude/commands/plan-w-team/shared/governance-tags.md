...
| Governor command execution                    | `.claude/scripts/pwt-governor-lib.sh`                                                                                    | Governor Contract phase 2. `pwt_governor_run_cmd`/`event_sink`/`liveness_cmd` execute a governor-supplied command string. A change here can widen arbitrary command execution — the git-tracked-manifest refusal, the required timeout, and the `env -i` argv exec are the load-bearing guards. Needs a security review.                                                          |
