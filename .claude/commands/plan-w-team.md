...
>
> - **Every Opus 4.8 pin is now 5.5.** That covers the verdict/supervisor floor (`team/evaluator`, `team/validator`, `team/supervisor`), every roster agent that pinned `claude-opus-4-8`, the `pwt-goal.sh`/`pwt-steer.sh` `PWT_PRIMARY_MODEL`/`PWT_FALLBACK_MODEL` defaults, the rate-limit step-down rung, the governor's `intelligent` default, the shell's non-lead model, and `fallbackModel`. The governor allow-list drops `claude-opus-4-8`: a stale governed config naming it is refused and lands on the 5.5 default. Opus 4.8 isn't kept as a rung either, since a previous-generation Opus is strictly dominated (see the v2 note).
...
> - **Unchanged:** the exact-string Opus 5 ban; the rate-limit step-down rung (`/model claude-opus-5-5`); the Haiku mechanical tier; the alias rules (`opus` is never a pin or seam value); the Opus 5.5 behavioral deltas in the v8 note.
