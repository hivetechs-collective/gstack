...
if [ -d "$WORKFLOW_LOCK_DIR" ]; then
  ...
  # other value: it proves nothing either way, as the janitor reads it too. One exception,
  # made by this pre-flight only (the janitor still counts such a lock as held): a
  # pre-row-191 lock (a `pid` file, no owner pid=) whose RAW pid, as read (no sign or
  # whitespace stripped), holds a character that is neither a digit nor whitespace
  # (`abc`, `-1`, `+5`, `-`, `+`) is reclaimed, "legacy pid names no process": that file
  # only ever held the pre-flight shell's `$$`. An empty or whitespace-only pid still
  # conflicts (another session may be creating that lock right now), and so does a
  # whitespace-padded decimal (` 123` may be a live pid).
  PWT_LK_OK=1; case "$O_PID" in ''|0*|1|*[!0-9]*) PWT_LK_OK=0 ;; esac
  PWT_LK_NAN=0; case "$O_PID" in *[!0-9[:space:]]*) PWT_LK_NAN=1 ;; esac
  PWT_LK_ACT="conflict"
  ...
  elif [ "$O_KIND" = "legacy" ] && [ "$PWT_LK_NAN" = 1 ]; then
    PWT_LK_ACT="reclaim"; PWT_LK_WHY="legacy pid names no process"
...
- **Conflict** (exit 1) in every other case, including an aged lock whose liveness query failed, whose answer left out this session, or whose session is live, an aged lock checked by a lead with no session id, a live owner pid whose start time or name `ps` cannot read (the message says which), and an owner pid that is not a decimal above 1 (empty, `0`, negative, `1`, or not a number; `kill -0 0` and `kill -0 -1` succeed, so such a pid would read as alive, and nothing about it shows the owner gone, which is how the janitor reads it too), except the one legacy reclaim below, which only this pre-flight makes: uncertainty keeps the lock. The message names the owner session and pid. The manual override is deleting the lock dir.
- A pre-row-191 lock (only a `pid` file, no owner `pid=`) is reclaimed exactly as before when that pid is dead. It is also reclaimed, "legacy pid names no process", when its RAW pid value, as read (no sign or whitespace stripped), holds a character that is neither a digit nor whitespace (`case "$O_PID" in *[!0-9[:space:]]*)`): `abc`, `-1`, `+5`, `-` and `+` all are, since that file only ever held the pre-flight shell's `$$`. An empty or whitespace-only `pid` still conflicts (another session may be creating that lock right now), so does a whitespace-padded decimal (` 123`, which may be a live pid), and a live decimal pid still holds. Only this pre-flight reclaims such a lock: the janitor (`__lock_hold_v`) still counts it as held (fail-closed).
...
**Model Tiering v9 — tiers by thinking depth (2026-09-22, skill 2.50.0).** Three tiers and **no Fable anywhere, fallbacks included** (operator ruling 2026-09-22): **Opus 5.5** (`claude-opus-5-5`) at **`high`** = the highest-thinking work — the bg worker and supervisor sessions, the judges (`evaluator`, `validator`), the reviewers, the hard-lane `builder-opus`, design and the spec consult; **Sonnet 5** (`claude-sonnet-5`) at `high` = medium-thinking, instruction-following build work — the routine `builder` lane and the Hands specialists; **Haiku 4.5** = no-thinking mechanical work (`file-scanner`, `log-parser`, `build-runner`). Every automated session runs Opus 5.5 at `high`; an interactive CLI session is the operator's own choice (ultracode). Every pipeline agent carries a literal pin again — none inherits — so Step 2's `difficulty` routing (routine → Sonnet 5, hard → Opus 5.5) is the per-task model decision. The consumer's shipyard still decides the bg **session** model per work item (`scripts/ops/model-tiers.json` → `dispatch-lane.sh` → `PWT_PRIMARY_MODEL`, default `claude-opus-5-5`); when Opus 5.5 is overloaded or unavailable that session steps down `claude-opus-4-8,claude-sonnet-5` (`pwt_fallback_model`, `pwt-governor-lib.sh`). **Opus 5 (`claude-opus-5`, exactly) stays forbidden; Opus 5.5 is not banned.** Parallelism is speed. Seams: `PWT_SUBAGENT_MODEL_BUILDER` / `PWT_SUBAGENT_MODEL_MECHANICAL` (see the v6 generation note below).

| Role                                | Tier  | Pinned Model                  | Agent Definition                                                                               | Rationale                                                                                                                                                                                                                                                                                                                  |
| ----------------------------------- | ----- | ----------------------------- | ---------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Lead (you)                          | Brain | Opus 5.5 (session)            | invoking session default                                                                       | Orchestration, judgment calls, scope decisions — **never `claude-opus-5`** (founder order 2026-08-29; Opus 5.5 is allowed, v8). Automated sessions: Opus 5.5 at `high` (v9); an interactive lead is the operator's own choice                                                                                                                                                                                                                            |
| Evaluator                           | Brain | `claude-opus-5-5`             | `.claude/agents/team/evaluator.md` frontmatter                                                 | Independent quality assessment                                                                                                                                                                                                                                                                                             |
| Fix-First Reviewer                  | Brain | `claude-opus-5-5`             | lead-invoked Pass 1/2 review                                                                   | Security review, one-way door scrutiny                                                                                                                                                                                                                                                                                     |
| Validator                           | Brain | `claude-opus-5-5`             | `.claude/agents/team/validator.md` frontmatter                                                 | Security-critical read-only review                                                                                                                                                                                                                                                                                         |
| Builder (hard lane)                 | Brain | `claude-opus-5-5`             | `.claude/agents/team/builder-opus.md` frontmatter                                              | `difficulty: hard` — novel/cross-cutting/ambiguous/security work. Pinned Opus 5.5 @ `high` (v9)                                                                                                                                                                                                                            |
| Builder (routine lane)              | Hands | `claude-sonnet-5`             | `.claude/agents/team/builder.md` frontmatter                                                   | Default lane — well-specified, instruction-following work. Pinned Sonnet 5 @ `high` (v9), as are the Hands specialists. Seam `PWT_SUBAGENT_MODEL_BUILDER` (`sonnet`/`haiku` only)                                                                                                                                          |
| Design agents                       | Brain | `claude-opus-5-5`             | `research-planning/system-architect.md`, `ui-designer.md`, `style-theme-expert.md` frontmatter | Architecture, UX/visual design — Opus 5.5 since v9 (Fable 5.1 retired). Read-only/advisory by definition; never a builder pool                                                                                                                                                                                             |
| Spec consult                        | Brain | `claude-opus-5-5`             | `.claude/agents/team/fable-spec-consult.md` frontmatter (name historical)                      | Step-1 §1b-bis read-only spec consult, spawned with the §1b-pre fan-out; no guard since v9. The Step-5 Fable top rung is **retired** — the ladder tops out at the hard lane, then the operator. Only `PLAN_W_TEAM_FABLE_REENABLE=1` revives the guard-gated rung, and its skip/refusal lands on Brain (Opus 5.5) — never lower, never `claude-opus-5`. |
| Mechanical                          | Haiku | `haiku`                       | `mechanical/file-scanner.md`, `log-parser.md`, `build-runner.md` frontmatter                   | No-thinking work: file scans, log parsing, build runs. Seam `PWT_SUBAGENT_MODEL_MECHANICAL` (`sonnet`/`haiku` only)                                                                                                                                                                                                        |
...
> **Generation note (2026-09-22, skill 2.50.0 — Model Tiering v8: Opus 5.5 is the intelligent floor, at xhigh):** _(the v9 note below supersedes its xhigh effort and its Fable-landing bullet; the Opus 5.5 rollover, the exact-string ban and the behavioral deltas stand.)_ Claude Opus 5.5 (`claude-opus-5-5`) shipped 2026-09-22 with Claude Code 2.1.280, and the operator ordered three things the same day: replace every Opus 4.8 pin with Opus 5.5; land Fable 5.1 fallbacks on Opus 5.5; run Opus 5.5 at **xhigh** by default. It also clarified that the v5 ban covers **exactly `claude-opus-5`**. Opus 5.5 was never banned and must never match a ban pattern, so bans are exact-string (never a `claude-opus-5*` glob). What moved:
>
> - **Every Opus 4.8 pin is now 5.5.** That covers the verdict/supervisor floor (`team/evaluator`, `team/validator`, `team/supervisor`), every roster agent that pinned `claude-opus-4-8`, the `pwt-goal.sh`/`pwt-steer.sh` `PWT_PRIMARY_MODEL`/`PWT_FALLBACK_MODEL` defaults, the rate-limit step-down rung (v9: now the first `pwt_fallback_model` rung — see below), the governor's `intelligent` default, the shell's non-lead model, and `fallbackModel`. The governor allow-list drops `claude-opus-4-8`: a stale governed config naming it is refused and lands on the 5.5 default. Opus 4.8 isn't kept as a rung either, since a previous-generation Opus is strictly dominated (see the v2 note).
> - **Fable landing tier = Opus 5.5.** The guard's SKIP and a Fable `stop_reason: "refusal"` both continue on 5.5. Fable 5.1 reads Opus 5.5 thinking blocks on the Claude API, so moving from 5.5 up to Fable keeps its reasoning. The reverse move (Fable→5.5) runs without Fable's blocks; the API drops them unbilled. `reasoning_extraction` refusals are never retried by server-side fallback.
> - **Effort: xhigh.** The pipeline effort pins and the bg `--effort` default go `high → xhigh`, and the operator's interactive `modelSettings.claude-opus-5-5.effortLevel` is `xhigh`. This is an escalation over the API default (`medium` on 5.5). Per Anthropic, 5.5 at `medium` matches or beats Opus 5 at `high` on agentic coding, and at the same level 5.5 thinks more per turn than Opus 5, most of all at `xhigh`/`max`. Expect longer turns and more output tokens. That cost is partly offset by 5.5's lower price ($4/$20 per MTok; cache reads $0.20 vs 4.8's $0.50) and by its >30% faster output. If burn becomes the constraint, step down to `high` first; don't swap the model.
> - **The `opus` alias.** From CLI 2.1.280 the bare `opus` alias resolves to `claude-opus-5-5`. On older CLIs it resolved to the forbidden `claude-opus-5`, so it stays unusable as a frontmatter pin and as a `PWT_SUBAGENT_MODEL_*` seam value (consumers may lag the CLI). The user-global `agent-model-guard.sh` allows it only when the transcript shows CLI ≥ 2.1.280, and fails closed otherwise.
> - **Behavioral deltas (5.5 vs Opus 5, none breaking for us).** Thinking can't be disabled; nothing in the pipeline disables it. Forced `tool_choice` is rejected; the pipeline never forces tools. Text between tool calls now arrives as progress-update `thinking` blocks, and Claude Code renders them. Bio and `reasoning_extraction` classifiers are new. In unattended runs 5.5 may end a turn with a progress report while work is still owed. The evaluator's "a text-only end of turn is a report, not proof of done" rule is exactly Anthropic's recommended harness fix.

> **Generation note (2026-09-22, skill 2.50.0 — Model Tiering v9: Opus 5.5 high / Sonnet 5 / Haiku; no Fable):** later the same day the operator replaced v8's effort and Fable rules: "we are going to use Opus 5.5 on high for all automated sessions that need the highest level of thinking effort. For the interactive sessions like this one where I'm using a CLI directly, I'll go ahead and override that and use Ultra Code. But for the default settings for the models, we will be using Opus 5.5 for the highest thinking, then Sonnet 5 for medium thinking, and then Haiku for no thinking." Fable 5.1 leaves every role and every fallback. What moved:
>
...
To control a subagent's model (v9 — three mechanisms):

1. **Hardcode a full model ID in frontmatter** (v9 — every pipeline agent): `claude-sonnet-5` for the routine `team/builder` and the Hands specialists; `claude-opus-5-5` for `team/builder-opus`, the verdict/supervisor roles, the reviewers, design and the spec consult; `haiku` for the mechanical tier. `model: inherit` is no longer used — an inheriting subagent takes the bg session's Opus model onto work the tiers put elsewhere. Never use the alias `opus` as a pin — it drifts with the CLI (`claude-opus-5-5` from 2.1.280, but the FORBIDDEN `claude-opus-5` on older CLIs a consumer may still run); a literal ID or `inherit` only.
2. **Do NOT set `model:` in the Agent tool call** unless honoring a validated `PWT_SUBAGENT_MODEL_*` seam. The param accepts only aliases (`opus`/`sonnet`/`haiku`); an alias overrides frontmatter, and `opus` is version-dependent (the forbidden Opus 5 before CLI 2.1.280). A seam value is honored ONLY when it is `sonnet` or `haiku` (see the v6 note above and `03-execute.md` Execution item 2).
3. **Effort pins (1.52.2, coverage completed 1.52.3; `team/fable-spec-consult` added 1.57.0)**: every /plan-w-team pipeline agent — `team/builder`, `team/builder-opus`, `team/evaluator`, `team/validator`, `team/supervisor`, `team/silent-failure-hunter`, `team/fable-spec-consult`, plus the two named Hands specialists (`react-typescript-specialist`, `rust-backend-specialist`) — carries an explicit `effort:` pin — **`high` since v9** (operator ruling 2026-09-22: Opus 5.5 runs at `high` in every automated session; v8's `xhigh` is retired). Subagent effort otherwise INHERITS the session effort (documented default), so an unpinned fan-out silently tracked whatever the lead happened to run — the effort-axis twin of the Fable model-default incident. Know the pin's exact semantics:
   - **The pin is bidirectional.** It blocks session bleed in both directions: a `/effort low` lead still runs pinned agents at `high`, and an ultracode or `max` lead does not push them higher. To deliberately run the pipeline at another level, edit the `effort:` pins in claude-pattern (consumer-repo edits are reverted on next sync). Under v9 the pin is still one step above Opus 5.5's API default (`medium`; Opus 5 and Sonnet 5 default to `high`). Anthropic reports 5.5 at `medium` already matches Opus 5 at `high` on coding, and that 5.5 thinks more per turn than Opus 5 at the same level, so `high` buys depth without v8's `xhigh` turn length. Bg workers get the same level through `pwt-goal.sh --effort` (`PWT_BG_EFFORT`, default `high`).
   ...
   - **Per-task depth on a pinned agent**: prompt phrasing (`ultrathink`, 04-fix-first REQ-3) deepens adaptive thinking WITHIN the pinned effort level; it is NOT verified to raise the effort parameter itself. The verified per-task escalation lever is a model bump: the hard-lane re-dispatch (Sonnet 5 → Opus 5.5). The guard-gated Fable rung that sat above it is retired under v9 (dormant behind `PLAN_W_TEAM_FABLE_REENABLE=1`; see `04-fix-first-review.md`).
4. For mechanical work done directly by the lead (ship, retro), no pinning is needed — the lead's session model is used. These phases are short (~5% of total tokens combined), so running them on the lead's Brain-tier model is not a meaningful cost concern. If you want to force Hands-tier for ship/retro, delegate to a `builder`-type subagent for the mechanical steps (note: the delegated builder runs on its pinned Sonnet 5 at `effort: high`, not the lead's session model or effort).

When a new model generation ships (rollovers applied: Opus 4.8 on 2026-05-28; Model Tiering v2 / Sonnet-5 Hands on 2026-07-09; Model Tiering v4 / Opus-5 Brain on 2026-07-24; Model Tiering v5 / Opus-5-forbidden on 2026-08-29; Model Tiering v6 / follow-the-lane on 2026-08-30; Model Tiering v8 / Opus-5.5 Brain + xhigh on 2026-09-22; Model Tiering v9 / Opus 5.5 high + Sonnet 5 builders + no Fable on 2026-09-22) — **under v9 every pipeline agent carries a literal pin again**, so a rollover is a spawn-site edit plus the frontmatter pins:

- **Spawn-site session model** (`pwt-goal.sh` / `pwt-steer.sh` `PWT_PRIMARY_MODEL`, the fallback chain in `pwt_fallback_model`, and the rate-limit step-down rung): the bg worker and supervisor sessions run on it. It must stay non-Fable — the anti-lockout floor.
- **The literal frontmatter pins to bump**: Opus — `team/builder-opus`, `team/evaluator`, `team/validator`, `team/supervisor`, `team/silent-failure-hunter`, `team/fable-spec-consult`, `research-planning/test-gap-analyzer`, `security-gap-analyzer`, `system-architect`, `ui-designer`, `style-theme-expert`, plus every roster agent pinned to `claude-opus-5-5`; Sonnet — `team/builder`, `implementation/react-typescript-specialist`, `rust-backend-specialist`; mechanical — `mechanical/file-scanner`, `log-parser`, `build-runner` (`haiku`).
- Do NOT demote a previous Opus into a lane — a previous-generation Opus carries the same usage weight as the current one with worse output (strictly dominated), which is why Opus 4.7 was retired. (v9's `claude-opus-4-8` is an availability fallback rung only, never a lane or a pin.) And never use the bare alias `opus` as a pin (it drifts with the CLI — it meant the forbidden Opus 5 before 2.1.280).
...
### Opus Practices (4.7 → 5.5)

Lead and Brain-tier agents follow the Opus patterns in `shared/opus-4-7-practices.md`:
...
- **Bounded subagent spawning** (INVERTED since Opus 5; Opus 5.5 sustains long runs with parallel subagents even better) — delegates readily; bound it rather than encourage it. Delegate only for disjoint-file tracks that each exceed a handful of tool calls; prefer one subagent over several; never delegate verification of your own work.
- **Auto mode + completion hooks** — let runs proceed without polling; rely on desktop notifications.
- **Default effort: high on Opus 5.5 for automated work** (v9 operator ruling; the API default is `medium`; interactive sessions are the operator's own, e.g. ultracode) — drop to medium only for narrow cost/latency-sensitive tasks.
