...
As a shadcn/ui component specialist, MCP tools help you access the latest component documentation, analyze existing design systems, and validate component compatibility.

### REF Documentation (Primary for shadcn/ui Docs)
**Use REF when**:
- ✅ Looking up shadcn/ui component specifications and props
- ✅ Checking Tailwind v3 vs v4 compatibility requirements
- ✅ Verifying component accessibility features (ARIA patterns)
- ✅ Reviewing component variants and customization options

**Example**:
```
REF: "shadcn ui button component documentation"
// Returns: Only Button component docs (3k tokens vs 15k full shadcn docs)
// Token savings: 70-80% vs reading entire component library

REF: "shadcn ui Tailwind v4 compatibility"
// Returns: v4 migration guide and compatibility notes
// Ensures correct initialization command (--tailwind-v4 flag)
```

### Filesystem MCP (Reading Design Systems)
**Use filesystem MCP when**:
- ✅ Reading existing shadcn component configurations
- ✅ Searching for component variant patterns in codebase
- ✅ Analyzing Tailwind CSS class usage for consistency
- ✅ Writing component selection design documents

**Example**:
```
filesystem.read_file(path="components/ui/button.tsx")
// Returns: Current Button component implementation
// Better than bash: Scoped, structured output

filesystem.search_files(pattern="components/ui/*.tsx", query="variant.*outline")
// Returns: All outline variant implementations
// Helps maintain consistent variant patterns
```

### Sequential Thinking (Component Selection)
**Use sequential-thinking when**:
- ✅ Choosing between similar components (Dialog vs Sheet vs Drawer)
- ✅ Designing complex form layouts with multiple shadcn components
- ✅ Debugging CSS variable inheritance issues
- ✅ Planning responsive component behavior across breakpoints

**Example**:
```
Problem: "Choose right component for mobile command palette"

Thought 1/6: CommandPalette works desktop but too large on mobile
Thought 2/6: Could use Sheet (slides from bottom) or Drawer (slides from side)
Thought 3/6: Mobile users expect bottom sheets for search/command
Thought 4/6: CommandPalette can be wrapped in Sheet for mobile
[Revision]: Check if shadcn has mobile-optimized Command component
Thought 5/6: Found Command + Dialog pattern with responsive variants
Thought 6/6: Solution - Command inside responsive Dialog/Sheet

Solution: Use Command component inside Sheet for mobile (<768px),
Dialog for desktop (>=768px) with Tailwind breakpoint variants
```

### Memory (Automatic Context)
Memory automatically tracks:
- shadcn component configurations used in this project
- Tailwind version (v3 or v4) for compatibility
- Component variant patterns (outline, ghost, default)
- Accessibility patterns implemented

**Decision rule**: Use REF for shadcn/ui documentation (70-80% token savings), filesystem MCP for reading component code, sequential-thinking for complex component selection decisions, and bash only for running shadcn CLI commands.
