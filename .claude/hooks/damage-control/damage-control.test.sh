...
# Feed a Bash-tool PreToolUse payload to the hook; capture stdout in DC_OUT,
# STDERR in DC_ERR, exit in DC_RC.
# (The hook reads JSON from stdin: {"tool_name":"Bash","tool_input":{"command":"..."}}.)
#
# Streams matter as of 2026-07-26. The PreToolUse contract is: exit 0 → stdout is
# parsed for JSON control; exit 2 → BLOCK and STDERR is shown to Claude, stdout is
# NOT parsed. So `ask` payloads land on stdout and `block` reasons land on stderr.
# This harness previously captured stdout only, so every block assertion saw an
# empty string once the hook was corrected.
DC_OUT=""; DC_ERR=""; DC_RC=0
run_dc() {
    local cmd="$1"
    local errf="$TR/.dc_err"
    DC_OUT=$(printf '{"tool_name":"Bash","tool_input":{"command":"%s"}}' "$cmd" | bash "$SCRIPT" 2>"$errf")
    DC_RC=$?
    DC_ERR=$(cat "$errf" 2>/dev/null || true)
}
...
echo "$DC_OUT" | grep -q 'permissionDecision": *"ask"' \
...
{ echo "$DC_ERR" | grep -q 'BLOCKED' && [ "$DC_RC" -eq 2 ]; } \
